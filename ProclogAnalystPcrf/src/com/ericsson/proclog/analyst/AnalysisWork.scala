...
class AnalysisWork(text: String) {
  ...
  private[this] var setOfMCA21: Set[CSOEntry] = TreeSet()
  private[this] type StrArr = Array[String]

  private[this] def isHeaderOfNBIA(fields: StrArr) = fields(0).contains("northbound") && fields(10).contains("SAPC@http://www.chinamobile.com/PCRF/")

  private[this] def isEndOfNBIA(fields: StrArr, id: String) = fields(0).contains("northbound") && !fields(1).contains(id)

  private[this] def isHeaderOfMCA21(fields: StrArr, id: String) = fields(0).contains("internal") && fields(1).contains(id)
  ...
  private def sortOutByLogIdForSouthBound(rawMCA21: String, id: String, cso: CSOEntry) {
    ...
    val mca21 = new MCA21Entry("internal", formatOper(fields), formatTime(fields), formatDuration(fields), mcs21Req, 2)
    cso.addMCA21(mca21)
  ...
  private def formatRequest(line: String) = {
    ...
    val req = fields(fields.length -1)
